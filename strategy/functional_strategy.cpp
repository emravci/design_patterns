...
class Shape
{
	...
	protected:
	using strategy_type = std::function<double(const Shape&)>;
};
...
class Circle : public Shape
{
	...
	virtual double cost() const override { return material_cost_strategy_(*this); }
	private:
	double radius_;
...
{
	Shapes shapes{};
	shapes.emplace_back(std::make_unique<Circle>(2.5, AluminumCostStrategy{}));
	shapes.emplace_back(std::make_unique<Square>(3.0, SteelCostStrategy{}));
	shapes.emplace_back(std::make_unique<Circle>(4.0, SteelCostStrategy{}));
